...
Pomona Finance — non-custodial auto-compounding yield vaults for Cardano LP positions (Minswap V2 first). **Status: pre-build.** Most source files are design-artifact skeletons or one-line stubs; `docs/decisions.md` is the authoritative design record (D1–D21; several early entries are marked SUPERSEDED — always check headers) and should be read before implementing anything, and updated (dated entries) when a design decision is made or changed. `docs/workflows/` holds the per-action implementation contracts (deposit, vault-init, compound, …) that sit between decisions.md and source — read the relevant one before coding a path.

...
**Archived 2026-07-30 (D26) with the rest of `legacy/`.** These are ideas
parked against the farm-emissions auto-compounding architecture specifically
— see `legacy/README.md`. The one entry that turned out to be
architecture-agnostic (CIP-26 registry) was carried forward into the current
`docs/v2-ideas.md` instead of staying only here.
...
**Idea:** anyone can init an ADApose vault for a new Minswap pool.
...
## ~~Individual per-user vaults alongside the pooled vault (dual product line)~~ RESOLVED — see D26

**RESOLVED 2026-07-30 (D26), by the pivot itself, not by building anything
below.** This entry's question was "should we offer a second, non-custodial
product line *alongside* today's pooled auto-compounding vault" — that
question no longer parses once the auto-compounding vault stopped being the
live product (this whole `legacy/` tree). More precisely: variant 1 below
(D17 — script-owned, no farm, genuinely non-custodial) isn't a parked
alternative anymore; it's close to what the new architecture's own target
custody model already is, per D26's own text ("no executor-address outputs,
unilateral in-kind exit"). Variant 2 is entirely farm-custody-specific and
lives on only as history here. Kept for the reasoning, not as a live idea.

**Idea:** offer a second, non-custodial product line next to today's pooled
auto-compounding vault — surfaced discussing whether ADApose can be described

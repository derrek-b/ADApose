# WingRiders V2 — on-chain findings (vendored 2026-07-17, Pool.hs added 2026-07-26)
...
| `Pool.hs` | Pool validator — `pparseRequest` does the actual request-datum validation `Request.hs` delegates to (deadline, deposit amounts, `oil` ADA-minimum check, receiver/compensation-datum handling). Vendored for the deposit-adapter design comparison in `docs/dex-adapters.md`. |
| `ConstantProduct.hs` | Constant-product pool math — `papplyAddLiquidity`/`paddLiquidityZapIn`/`paddLiquidity`: the actual deposit LP-mint formula, incl. the single-sided-zap-in-only vs. two-sided-no-swap-haircut split. Also vendored for `docs/dex-adapters.md`. |

#!/usr/bin/env python3
"""
Offline end-to-end exercise of `measure` for the Minswap V2 venue.
...
class MockBF(S.Blockfrost):
    ...
    def asset_txs_page(self, asset, page, order="desc", count=100):
        ...
        # about it would return every pool on the DEX. The real client cannot
        # detect that; the mock can, so it refuses.
        assert asset != NFT, (
            "measure paged history by the VENUE-WIDE NFT -- that returns every "
            "Minswap pool interleaved. It must page by the pool's LP asset.")
    ...
    def tx_utxos(self, tx_hash):
        ...
        held = MAXB - lp + S.VENUES["minswap-v2"].lp_locked
        if BREAK_CROSSCHECK:
...
def run(tag, pool_overrides=None):
    pool = {"label": "MIN2-ADA-MIN", "venue": "minswap-v2", "nft": NFT,
            "script_hash": SH, "lp_asset": LP, "track_asset": LP,
            "asset_a": "lovelace", "asset_b": TOKB}
    pool.update(pool_overrides or {})
    with open("/tmp/mock_min_pools.json", "w") as fh:
        json.dump([pool], fh)
    ...
    out_path = f"/tmp/mock_min_{tag}.csv"
    if os.path.exists(out_path):
        os.remove(out_path)  # measure() now APPENDS -- start each mock clean
    args = types.SimpleNamespace(env_file=".env", pools="/tmp/mock_min_pools.json",
                                 out=out_path,
                                 days="7,14,30,60", quiet=True)
    print(f"\n########## {tag} ##########")
    try:
...
def main():
    ...
    # ---- 1. correct config ------------------------------------------------
    rc, calls = run("correct")
    print(f"\nAPI calls for 1 pool x 4 lookback points: {calls}")
    if rc != 0:
        fails.append("correct config should exit 0")
    rows = list(csv.DictReader(open("/tmp/mock_min_correct.csv")))
    expected = (DAILY ** 365 - 1) * 100
    print(f"expected fee APR ~{expected:.3f}%")
    for r in rows:
        got = float(r["fee_apr_pct"])
        ok = abs(got - expected) < 0.05
        print(f"  {r['days']:>6}d window -> {got:.4f}%  lp={r['lp_supply_to']}  "
              f"{'OK' if ok else 'MISMATCH'}")
        if not ok:
            fails.append(f"APR mismatch on {r['days']}d window: {got} vs {expected}")
    # Every window ENDS at T-0, so lp_supply_to is identical in all four rows by
    # construction -- comparing those proves nothing. Read the LP supply at the
    # two ends of the 60d window directly instead: it must differ, because a
    # deposit lands on day 370, inside that window. That the 60d APR still comes
    # back at exactly the baked-in rate is the actual proof that a mid-window
    # deposit does not move sqrt(k)/LP.
    mock2 = MockBF()
    pools = S.load_pools("/tmp/mock_min_pools.json")
    venue = S.VENUES["minswap-v2"]
    lps, walked = [], []
    for off in (60, 0):
        st, skipped = None, 0
        for cand in S.iter_txs_at_or_before(mock2, pools[0].identity,
                                            NOW - off * DAY, verbose=False):
            st = S.read_state(mock2, pools[0], venue, cand, {}, NOW - 60 * DAY,
                              verbose=False)
            if st:
                break
            skipped += 1
    ...
    if rc == 0:
        ...
        print("LP cross-check fired as intended (non-zero exit)")
    ...
    if rc == 0:
        ...
        print("rejected pool output was reported, not walked past (non-zero exit)")

    # ---- 3. a Minswap pool with no track_asset must be refused -----------
    print("\n########## missing track_asset ##########")
    try:
        rc, _ = run("no_track", {"track_asset": ""})
